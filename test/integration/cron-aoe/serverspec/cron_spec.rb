...
describe "AOE Cron" do
  describe file("/etc/cron.d/magento-site1") do
    ...
    its(:content) { should match /.*\* \* \* \* \* apache sh \/docroot\/scheduler_cron.sh --mode default --excludeGroups excludegroup.*/ }
    its(:content) { should match /.*\* \* \* \* \* apache sh \/docroot\/scheduler_cron.sh --mode default --includeJobs includejob.*/ }
  end
end